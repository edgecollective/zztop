/*
 * A second frequency: the first spectroscopy.
 *
 * The engine is identical to the previous application. Only the excitation frequency
 * changes, from an auto-reload of 937 to 9374, which gives a 25.6 kHz sample rate and
 ...
 *
 * The test network here is a resistor in parallel with a capacitor, and two competing
 * models of it were on the table: an ideal 470 nF part, and a back-solved 322 nF with
 * about 27 ohms of equivalent series resistance. At the original kilohertz point those
 * models are hard to separate. At 100 Hz their predicted phases differ by roughly ten
 * degrees, which is far larger than the residual offset characterised in the previous
 * application, so the measurement can distinguish them.
 *
 * This is the point where the instrument stops demonstrating that it works and starts
 * being used to answer a question about something else.
 */
...
#define LUT_LEN   256
#define TIM6_PSC  0
#define TIM6_ARR  9374       /* f_s = 240 MHz / 9375 = 25.6 kHz -> f_out = 100.0 Hz */
#define N_PERIODS 10
...
static int burst_capture(int *idx0_out)
{
	...
	*idx0_out = (int)((LUT_LEN - ndtr) % LUT_LEN);
	...
	while (__HAL_DMA_GET_COUNTER(&hdma_adc1) > 0) {
		if (k_uptime_get() - t0 > 500) {           /* burst takes ~100 ms at 25.6 kHz */
...
int main(void)
{
	...
	printk("excitation: f_s=25600 Hz  f_out=100.0 Hz on PA4/pin30 (second spectral point)\n");
